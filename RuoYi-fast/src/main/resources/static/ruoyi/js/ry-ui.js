...
(function ($) {
    $.extend({
        ...
        // 操作封装处理
        operate: {
			editUrlNoTable: function(id) {
				var url = "/404.html";
				if ($.common.isNotEmpty(id)) {
					url = table.options.updateUrlNoTable.replace("{id}", id);
				} else {
					$.modal.alertWarning("请输入商品条码");
					return;
				}
				return url;
			},
			editNoTable: function(id) {
				if($.common.isEmpty(id)) {
					$.modal.alertWarning("请输入商品条码");
				} else {
					$.modal.open(table.options.modalName, $.operate.editUrlNoTable(id));
				}
			},
        	// 提交数据
            	...
            	var config = {
        	        ...
        	        success: function(result) {
        	        	...
        	        	if( typeof result === 'string'){
							$.operate.ajaxSuccess(JSON.parse(result));
						}else{
			...
			submitNoTable: function(url, type, dataType, data, callback) {
				var config = {
					url: url,
					type: type,
					dataType: dataType,
			...
			submitForJson: function(url, type, dataType, data, callback) {
				var config = {
					url: url,
					type: type,
					dataType: dataType,
			...
			dispersed: function(id, width, height) {
				table.set();
				var _url = $.operate.detailUrl(id);
				var _width = $.common.isEmpty(width) ? "800" : width;
				var _height = $.common.isEmpty(height) ? ($(window).height() - 50) : height;
				...
				$.modal.open("折零出售", $.operate.detailUrl(id));
			},
            // 详细访问地址
			...
			unlock: function(id) {
				table.set();
				$.modal.confirm("确定解冻该条" + table.options.modalName + "信息吗？", function() {
					var url = $.common.isEmpty(id) ? table.options.unlockUrl : table.options.unlockUrl.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
			...
			procurement: function(id){
				table.set();
				$.modal.open("商品零售价设置", $.operate.procurementUrl(id));
			},
			// 批量删除信息
			procurementAll: function() {
				table.set();
				var rows = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
				if (rows.length == 0) {
					$.modal.alertWarning("请至少选择一条记录");
				...
				$.modal.confirm("确认要删除选中的" + rows.length + "条数据吗?", function() {
					var url = table.options.procurementUrl;
					var data = { "ids": rows.join() };
					$.operate.submit(url, "post", "json", data);
				});
			},
			...
			// 添加访问地址
			procurementUrl: function(id) {
				var url = $.common.isEmpty(id) ? table.options.procurementUrl.replace("{id}", "") : table.options.procurementUrl.replace("{id}", id);
				return url;
			},
            // 修改信息
            edit: function(id) {
            ...
            ajaxSuccess: function (result) {
				...
				console.log(web_status.SUCCESS == result.code)
